-- ====================================================================
-- MPLADS INTELLIGENCE: Analytical Tables Population & RLS Policies Migration
-- Database: Supabase PostgreSQL
-- Records: 100 project_financials, 100 project_progress, 100 risk_scores, 20 duplicate_matches
-- ====================================================================

-- STEP 1: Ensure Minimum Required RLS Policies for Anonymous/Public Client Access
DO $$
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'project_financials' AND policyname = 'Allow public insert project_financials') THEN
    CREATE POLICY "Allow public insert project_financials" ON project_financials FOR INSERT WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'project_financials' AND policyname = 'Allow public update project_financials') THEN
    CREATE POLICY "Allow public update project_financials" ON project_financials FOR UPDATE USING (true);
  END IF;
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'project_progress' AND policyname = 'Allow public insert project_progress') THEN
    CREATE POLICY "Allow public insert project_progress" ON project_progress FOR INSERT WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'project_progress' AND policyname = 'Allow public update project_progress') THEN
    CREATE POLICY "Allow public update project_progress" ON project_progress FOR UPDATE USING (true);
  END IF;
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'risk_scores' AND policyname = 'Allow public insert risk_scores') THEN
    CREATE POLICY "Allow public insert risk_scores" ON risk_scores FOR INSERT WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'risk_scores' AND policyname = 'Allow public update risk_scores') THEN
    CREATE POLICY "Allow public update risk_scores" ON risk_scores FOR UPDATE USING (true);
  END IF;

  -- duplicate_matches policies
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'duplicate_matches' AND policyname = 'Allow public insert duplicate_matches') THEN
    CREATE POLICY "Allow public insert duplicate_matches" ON duplicate_matches FOR INSERT WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'duplicate_matches' AND policyname = 'Allow public update duplicate_matches') THEN
    CREATE POLICY "Allow public update duplicate_matches" ON duplicate_matches FOR UPDATE USING (true);
  END IF;
END $$;

-- STEP 2: Clear any potential stale records before idempotent population
DELETE FROM project_financials;
DELETE FROM project_progress;
DELETE FROM risk_scores;
DELETE FROM duplicate_matches;

-- STEP 3: Seed project_financials (100 rows)
...
-- STEP 4: Seed project_progress (100 rows)
...
-- STEP 5: Seed risk_scores (100 rows)
INSERT INTO risk_scores (project_id, total_risk_score, progress_expenditure_score, model_name, model_version, prediction, top_risk_factors, recommendation, risk_level, cost_anomaly_score, payment_anomaly_score, delay_score, duplicate_similarity_score, input_timestamp)
VALUES
...
-- STEP 6: Seed duplicate_matches (20 rows)
...
SELECT
  (SELECT count(*) FROM projects) as count_projects,
  (SELECT count(*) FROM project_financials) as count_financials,
  (SELECT count(*) FROM project_progress) as count_progress,
  (SELECT count(*) FROM risk_scores) as count_risk_scores,
  (SELECT count(*) FROM duplicate_matches) as count_duplicate_matches;
